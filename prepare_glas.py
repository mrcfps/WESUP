...
def prepare_patches(orig_path, dst_path, names):
    ...
    for name in names:
        ...
        x.append(divide_image_to_patches(img, config.PATCH_SIZE, pad=False))
        y.append(divide_image_to_patches(mask, config.PATCH_SIZE, pad=False))

    x = np.concatenate(x)
    y = np.concatenate(y)

    for idx, (img_patch, mask_patch) in enumerate(zip(x, y)):
        imsave(os.path.join(dst_img_dir, f'patch-{idx}.bmp'), np.squeeze(img_patch))
        imsave(os.path.join(dst_mask_dir, f'patch-{idx}.bmp'), np.squeeze(mask_patch))
