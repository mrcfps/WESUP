...
gts = executor(delayed(imread)(gt_path) for gt_path in sorted(glob.glob('LUSC/test/masks/*.png')))
...
compute_metrics(predictions, gts, pred_paths)
# metrics.to_csv(osp.join(pred_root, 'metrics.csv'))
